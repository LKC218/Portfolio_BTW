...
const GridOverlay: React.FC<GridOverlayProps> = ({ accentColor, hotspotFocus }) => {
  // Use provided color or fallback to default Cyan/Red theme
  // If hotspotFocus is active, force a high-contrast "Analysis" color (White/Cyan mix)
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 bg-background overflow-hidden transition-colors duration-500">
      ...
      <div className="absolute inset-0 transition-all duration-1000 ease-in-out opacity-60 dark:opacity-40 mix-blend-multiply dark:mix-blend-screen">
         {/* Top Left Blob */}
         <div 
            className="absolute top-[-20%] left-[-20%] w-[70vw] h-[70vw] blur-[80px] animate-[orb-float-fast_5s_ease-in-out_infinite] transition-colors duration-1000" 
            style={{ background: `radial-gradient(circle, ${primaryColor}66 0%, transparent 60%)` }}
         />
         
         {/* Bottom Right Blob */}
         <div 
            className="absolute bottom-[-20%] right-[-20%] w-[80vw] h-[80vw] blur-[90px] animate-[orb-float-fast_7s_ease-in-out_infinite_reverse] transition-colors duration-1000" 
            style={{ background: `radial-gradient(circle, ${secondaryColor}4D 0%, transparent 60%)` }}
         />
         
         {/* Core Glow */}
         <div 
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[60vw] blur-[100px] animate-pulse transition-colors duration-1000"
            style={{ background: `radial-gradient(circle, ${primaryColor}0D 0%, transparent 70%)` }}
         />
      </div>

      {/* === 2. DIGITAL NOISE TEXTURE === */}
      <div className="absolute inset-0 opacity-[0.02] dark:opacity-[0.08] z-0" 
           style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }}>
      </div>

      {/* === 3. TOPOGRAPHIC CONTOUR TEXTURE (Replaces Grid) === */}
      {/* 
          Using SVG filters to create distorted lines simulating topography.
          Invert filter is used in light mode to make white lines dark.
      */}
      <div className="absolute inset-0 opacity-[0.12] dark:opacity-[0.08] invert dark:invert-0 pointer-events-none mix-blend-multiply dark:mix-blend-plus-lighter"
           style={{
      ...
      <div 
        className="absolute w-[40vw] h-[40vw] md:w-[20vw] md:h-[20vw] border border-foreground/5 rounded-full flex items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]"
        style={{ 
      ...
      {/* Vignette */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(var(--background),0.5)_90%)] dark:bg-[radial-gradient(circle_at_center,transparent_0%,rgba(var(--background),0.8)_90%)] z-10 pointer-events-none"></div>
    </div>
  );
};
